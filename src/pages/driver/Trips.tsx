...
import type { TripStatus } from '../../../shared/types.js';
...
export default function DriverTrips() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await api.driver.trips() as unknown as { list: TripRow[]; total: number };
        setList(res.list || []);
  ...
  const filtered = list.filter((t) => {
    ...
    if (keyword) {
      ...
      return t.origin.toLowerCase().includes(kw) || t.destination.toLowerCase().includes(kw) || (t.plateNumber || '').toLowerCase().includes(kw);
    }
    return true;
  });
      ...
      <div className="card">
        ...
        {filtered.length === 0 ? (
          ...
          <div className="space-y-3">
            {filtered.map((t) => {
              const st = t.status;
              const planMin = Math.max(10, Math.round(Math.abs(new Date(t.endTime).getTime() - new Date(t.startTime).getTime()) / 60000));
              return (
                <div key={t.tripId} className={`p-5 rounded-xl border-2 transition-all hover:shadow-md ${st === 'completed' ? 'border-success-100' : st === 'departed' ? 'border-accent-200 bg-accent-50/20' : 'border-slate-100'}`}>
                    ...
                    <div className="flex items-start gap-4 flex-1 min-w-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <h4 className="text-base font-bold text-primary-900 truncate">{t.origin} → {t.destination}</h4>
                          <span className={`tag-pill ${tripStatusColor[st]}`}>{tripStatusLabel[st]}</span>
                          <span className="tag-pill bg-primary-100 text-primary-700 text-[10px] font-mono">#{String(t.tripId).padStart(6, '0')}</span>
                        </div>
                        <div className="flex items-center gap-4 text-xs text-slate-500 flex-wrap">
                          <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {formatDateTime(t.startTime)} ~ {formatDateTime(t.endTime).slice(11)}</span>
                          {st === 'completed' && t.actualMileage != null && <span className="flex items-center gap-1 text-success-600"><Gauge className="w-3 h-3" /> 行驶 {t.actualMileage} km</span>}
                          <span className="font-mono font-semibold text-primary-700">{t.plateNumber}</span>
                        </div>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-2xl font-black text-primary-800 font-mono">{formatMoney(t.actualCost ?? 0)}</div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-100">
                      ...
                      <div className="text-sm font-bold text-primary-800">{formatDuration(planMin)}</div>
                    </div>
                    {st === 'completed' && t.actualDurationMin != null && (
                      <div className="p-3 rounded-lg bg-success-50 border border-success-100">
                        <div className="text-[11px] text-slate-500 mb-1">实际时长</div>
                        <div className="text-sm font-bold text-success-700">{formatDuration(t.actualDurationMin)}</div>
                      </div>
                    )}
                    {st === 'completed' && t.actualMileage != null && (
                      <div className="p-3 rounded-lg bg-success-50 border border-success-100">
                        <div className="text-[11px] text-slate-500 mb-1">实际里程</div>
                        <div className="text-sm font-bold text-success-700">{t.actualMileage} km</div>
                      </div>
                    )}
                    <div className="p-3 rounded-lg bg-slate-50 border border-slate-100">
                      ...
                      <div className="text-sm font-bold text-warning-600">待评价</div>
                    </div>
                  </div>
                </div>
              );
